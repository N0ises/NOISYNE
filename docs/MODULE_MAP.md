...
All implementation lives under `phasenox`. The top-level `brain` package is a
...
| --- | --- | --- |
| `phasenox/runtime/` | Model lifecycle, repository resolution, devices, loading, cache and runtime capability registry | Infrastructure/providers only; domain code does not load models directly |
| `phasenox/infrastructure/` | Configuration, provider plumbing and infrastructure concerns | No UI ownership |
| `phasenox/audio/` | Audio I/O, measurements, context, embeddings, comparison, V1 mix/plugin intelligence and audio-domain models | May use runtime; does not depend on Desktop/DAW protocols |
| `phasenox/engineering/` and `phasenox/audio/engineer/` | Existing rule/engineering analysis paths | Consume audio-domain evidence |
| `phasenox/reference/` | Reference comparison, reasoning, pipeline and reports | Consumes analysis/context; no DAW control |
| `phasenox/knowledge/` | Structured knowledge loading, validation, registry and resolution | Domain knowledge contracts |
| `phasenox/rag/` | Retrieval, ingestion, vector database and reranking | Optional model/data availability |
| `phasenox/memory/` | Memory/profile models, resolution and vector storage | Persistence identifiers remain compatibility-sensitive |
| `phasenox/providers/` | Replaceable AI provider contracts, registry, factory and services | Heavy/remote providers remain optional |
| `phasenox/reasoning/` and `phasenox/prompt/` | Structured reasoning, prompts, parsing and guards | Depend on contracts, not UI |
| `phasenox/report/` | Report models, building, validation and export | Consumes domain results |
| `phasenox/evaluation/` | Metrics, scoring, benchmarks and evaluation reports | Tests domain/application outputs |
| `phasenox/perception/` | V2 perceptual contracts, auditory frontend, calibrated loudness, relative simultaneous masking, descriptors, explicit playback transfer, and policy-conditioned translation evidence | Root contracts remain lightweight; numerical runtimes use existing audio/NumPy dependencies and have no providers, UI, API or DAW dependencies |
| `phasenox/application/` | Canonical application facade and use-case orchestration | May compose domain modules; no Desktop imports |
| `phasenox/orchestration/` and `phasenox/pipeline/` | Implemented alternate orchestration/stage infrastructure | Not the frozen V1 CLI production path |
| `phasenox/integration/` | Deterministic DAW-named workflow export contracts | No live DAW communication or control |
| `phasenox/cli.py` | Canonical command-line surface | Calls application services |

`phasenox.application.phasenox_service.PhasenoxService` is the canonical V1
facade. `phasenox.application.noisyne_service.NoisyneService` and
`phasenox.application.soundbrain_service.SoundBrainService` are retained
...
Sprint 1 established `phasenox/perception/` as the canonical package for V2
...
`phasenox/perception/auditory.py` and its minimal JSON-safe metadata contracts in
`phasenox/perception/auditory_contracts.py`. The Sprint 1 schema remains
...
Sprint 3 adds `phasenox/perception/loudness_contracts.py` and
`phasenox/perception/loudness.py`. This is an explicit digital-to-pascal
...
Sprint 4 adds `phasenox/perception/masking_contracts.py` and
`phasenox/perception/masking.py`. It computes pairwise, common-gain relative
...
Sprint 5 adds `phasenox/perception/descriptor_contracts.py` and
`phasenox/perception/descriptors.py`. The contracts classify standardized
...
Sprint 6 adds `phasenox/perception/transfer_contracts.py` and
`phasenox/perception/transfer.py`. It separates playback context from explicit,
...
Sprint 7 adds `phasenox/perception/translation_contracts.py` and
`phasenox/perception/translation.py`. It compares an original signal with the
...
`phasenox/integration` owns file-export contracts only. The Sprint 20 Ableton
...
- Historical documents may retain former SoundBrain naming; current modules and
  new code use PHASENØX/PHASENOX and `phasenox`.
