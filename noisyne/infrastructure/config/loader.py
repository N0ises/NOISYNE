...
from noisyne.infrastructure.config.settings import DEFAULT_SETTINGS
...
def get_application_root() -> Path:
    ...
    4. The parent directory of the installed ``noisyne`` package (e.g.
    ...
    config_module = sys.modules.get("noisyne.infrastructure.config")
    if config_module is None:
        raise RuntimeError(
            "Cannot determine NOISYNE application root: "
            "noisyne.infrastructure.config has not been imported."
        )
    ...
    # .../site-packages/noisyne/infrastructure/config. The parent of the ``noisyne``
...
def _load_yaml(name: str) -> dict[str, Any]:
    ...
    resource = resources.files("noisyne.infrastructure.config").joinpath(
