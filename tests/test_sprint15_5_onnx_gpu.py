...
def test_runtime_availability_cuda_truthful() -> None:
    """check_runtime_availability reports CUDA availability for ORT accurately."""
...
def test_provider_preference_prefers_cuda_first() -> None:
    """When CUDA is available, the fixture CUDA session uses it before CPU fallback."""
    if not _onnxruntime_cuda_available():
        pytest.skip("CUDAExecutionProvider not advertised")
    import onnxruntime as ort
...
def test_cpu_fallback_when_cuda_unavailable() -> None:
    """When CUDA is not available, CPU-only fixture runtimes still work."""
    if _onnxruntime_cuda_available():
        pytest.skip("CUDA is available; this test covers the CPU-only path")
...
def test_fixture_benchmark_timing_is_finite(fixture_runtimes: dict) -> None:
    ...
    from noisyne.performance import capture_environment
...
def test_capability_truth_separates_package_and_gpu(fixture_runtimes: dict) -> None:
    ...
    # Having the package does not imply CUDA loads, and advertised provider does
    # not imply execution worked.  The relationships are documented, not forced.
    assert isinstance(package_installed, bool)
