...
from phasenox.ui.contracts import AnalysisViewResult, OperationHandle, ReportDescriptor
from phasenox.ui.presentation_state import PageId, PresentationState, SessionState
from phasenox.ui.presentation_store import PresentationStore
from phasenox.ui.session_persistence import SessionPersistenceBinding, SessionRepository
...
def test_v2_session_migrates_with_v2_fields_at_safe_defaults(tmp_path) -> None:
    repository = SessionRepository(tmp_path / "session.json")
    repository.path.write_text(
        json.dumps(
            {
