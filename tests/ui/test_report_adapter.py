...
from brain.ui.contracts import ReportDescriptor, ReportExportCommand
...
def test_json_preview_is_safe_pretty_utf8_with_real_filesystem_metadata(
    ...
    preview = V1ApplicationAdapter(metadata=product_metadata).load_report(_descriptor(path))
...
def test_reference_markdown_preview_is_plain_text(product_metadata, tmp_path) -> None:
    ...
    preview = V1ApplicationAdapter(metadata=product_metadata).load_report(descriptor)
...
def test_export_copies_exact_bytes_and_preserves_descriptor_truth(
    ...
    result = V1ApplicationAdapter(metadata=product_metadata).export_report(
