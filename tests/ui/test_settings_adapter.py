...
def test_v2_settings_snapshot_exposes_effective_configuration_read_only(
    product_metadata,
) -> None:
    snapshot = V2ApplicationAdapter(metadata=product_metadata).settings_snapshot()
    values = {item.key: item for item in snapshot.values}

    assert snapshot.source == "packaged_default"
    assert snapshot.revision == "packaged-v2"
    assert not snapshot.credential_configured
    assert {
        "llm.provider",
        "llm.model",
        "llm.base_url",
...
def test_v2_settings_snapshot_never_exposes_secret_value_or_key_name(product_metadata) -> None:
    snapshot = V2ApplicationAdapter(metadata=product_metadata).settings_snapshot()
