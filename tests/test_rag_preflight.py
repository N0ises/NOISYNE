...
from phasenox.rag.ingestion import _chunk_id, _source_fingerprint, ingest_chunks
from phasenox.rag.scoring import score_from_distance
...
def test_retrieval_scores_are_valid_cosine_similarities(
    ...
    from phasenox.rag import retriever
...
def test_ingest_chunks_fail_when_stale_deletion_fails():
    ...
    from phasenox.rag.errors import IngestionError
...
def test_ask_reuses_search_results_without_second_retrieval(monkeypatch):
    ...
    monkeypatch.setattr("phasenox.services.rag_service.retrieve", fake_retrieve)
    monkeypatch.setattr("phasenox.services.rag_service.rerank", fake_rerank)
...
def test_persistence_paths_are_absolute_and_cwd_independent():
    from phasenox.audio.catalog.database import DATABASE_PATH
    from phasenox.memory.vector.config import PERSIST_DIRECTORY
